...
const ClienteItem = React.memo(({ cliente, seleccionado, onClick }) => {
    return (
        <ListItem
            ...
            sx={{
                mb: 1, borderRadius: 1,
                border: `1px solid ${seleccionado ? cliente.color || '#81c784' : 'transparent'}`,
                bgcolor: seleccionado ? alpha(cliente.color || '#81c784', 0.1) : 'transparent',
                '&:hover': { bgcolor: alpha(cliente.color || '#81c784', 0.15) }
            }}
        >
            <ListItemAvatar>
                <Avatar sx={{ bgcolor: cliente.color || '#81c784', width: 36, height: 36 }}>
            ...
            />
            {seleccionado && <ArrowRightAltIcon sx={{ color: 'success.main' }} />}
        </ListItem>
    );
});

// Componente SeccionHorarioJiujutsu mejorado
const SeccionHorarioJiujutsu = React.memo(({
    horario,
    diaSeleccionado,
    clientesAsignados = [],
    clienteSeleccionado,
                        ...
                        {clientesAsignados.map((cliente) => (
                            <ListItem key={`${cliente.id}-${diaHorarioKey}`} divider>
                                ...
                                <ListItemText
                                    ...
                                    secondary={`Cinturón: ${cliente.belt || 'N/A'}`}
...
const Jiujutsu = () => {
    ...
    // Función para generar colores pasteles verdes
    const getRandomColor = useCallback((index) => {
        const colors = ['#a5d6a7', '#81c784', '#66bb6a', '#4caf50', '#43a047', '#388e3c'];
        ...
        const cargarDatos = async () => {
            try {
                ...
                setState(prev => ({
                    ...
                    clientesJiujutsu: clientesFiltrados,
    ...
    const handleRemoveCliente = (diaHorarioKey, clienteId) => {
        ...
        updateState({
            ...
            snackbar: {
                open: true,
                message: 'Alumno eliminado del horario',
                severity: 'info'
            }
        });
    };
    ...
    if (state.loading) {
        return (
            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '80vh' }}>
                ...
                <Typography sx={{ ml: 2 }}>Cargando clases de Jiujutsu...</Typography>
            </Box>
        );
    }

    if (state.error && !Object.keys(state.asignaciones).length && state.clientesJiujutsu.length === 0) {
        return (
        ...
        <Container maxWidth="xl" sx={{ mt: 4, mb: 8 }}>
            ...
            <Box sx={{ mb: 4 }}>
                <Typography variant="h4" component="h1" gutterBottom align="center" sx={{ 
                    ...
                    <SportsMmaIcon fontSize="large" color="success" /> 
                    Gestión de Clases de Jiujutsu
                </Typography>
                <Typography variant="subtitle1" color="text.secondary" align="center">
                    Asigna alumnos a cada horario de entrenamiento de Jiujutsu
                </Typography>
            </Box>
            ...
            <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={3}>
                    <Paper elevation={2} sx={{ 
                        ...
                        bgcolor: '#f5faf5' // Fondo pastel verde claro
                    }}>
                        ...
                        {state.clienteSeleccionado && (
                            <Alert severity="info" sx={{ mb: 1.5 }} icon={<CheckCircleIcon fontSize="small" />}>
                                ...
                                <Typography variant="caption">
                                    Cinturón: {state.clienteSeleccionado.belt}
                                </Typography>
                            </Alert>
                        )}
                        ...
                        <Box sx={{ 
                            ...
                            {state.clientesJiujutsu.length > 0 ? (
                                <List dense sx={{ pt: 0 }}>
                                    {state.clientesJiujutsu.map(cliente => (
                                        <ClienteItem
                                            key={cliente.id}
                                            cliente={cliente}
                ...
                <Grid item xs={12} md={9}>
                    <Paper elevation={2} sx={{ borderRadius: 2, overflow: 'hidden', bgcolor: '#f5faf5' }}>
                        {/* Tabs de días */}
                        <Box sx={{ bgcolor: '#e8f5e9', borderBottom: '1px solid #c8e6c9' }}>
                            <Tabs
