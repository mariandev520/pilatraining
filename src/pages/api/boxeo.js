...
const ACTIVIDAD_KEY = 'boxeo'; // Clave para la colección y el tipo de documento
...
export default async function handler(req, res) {
  try {
    ...
    // Usamos una colección específica para funcional o una genérica con filtro
    // Opción 1: Colección específica
    const colFuncional = db.collection(ACTIVIDAD_KEY);
    ...
    const filtroDocumento = { tipo: ACTIVIDAD_KEY }; // Filtro para encontrar el doc de funcional

    // Método GET: Obtener asignaciones de Funcional
    if (req.method === 'GET') {
      // Buscar el documento único para las asignaciones de funcional
      // Usar colFuncional o colAsignaciones según la opción elegida arriba
      const funcionalDoc = await colFuncional.findOne(filtroDocumento);
      ...
      return res.status(200).json(funcionalDoc.asignaciones || {});
    }

    // Método POST: Guardar/actualizar asignaciones de Funcional
    if (req.method === 'POST') {
      ...
      // Usar colFuncional o colAsignaciones según la opción elegida
      const updateResult = await colFuncional.updateOne(
        filtroDocumento, // Filtro para encontrar el doc de funcional
        {
          $set: {
