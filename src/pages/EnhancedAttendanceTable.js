...
// Paleta de colores MODO OSCURO (Original)
// const DARK_COLORS = {
...
const LIGHT_COLORS = {
  backgroundDefault: '#F4F6F8',      // Gris muy claro para el fondo general
  backgroundPaper: '#FFFFFF',       // Blanco para elementos como tablas, cards
  backgroundSubtle: '#E9EFF3',      // Un gris un poco más oscuro que el paper, para hovers o elementos anidados
  textPrimary: '#212529',          // Negro/Gris oscuro para texto principal
  textSecondary: '#6C757D',        // Gris medio para texto secundario
  accentGreen: '#00B85E',          // Un verde vibrante, podría ser el mismo o uno optimizado para fondo claro
  accentGreenHover: '#009A4D',      // Verde más oscuro para hover
  divider: 'rgba(0, 0, 0, 0.12)',   // Divisor estándar para temas claros
  errorLight: '#F44336',           // Rojo para errores (versión más clara para texto/iconos)
  errorMain: '#D32F2F',            // Rojo principal para errores
  greyInactive: '#B0BEC5',         // Gris claro para elementos inactivos
  white: '#FFFFFF',                // Blanco puro
  black: '#000000',                // Negro puro
};

// Usaremos LIGHT_COLORS para este componente
...
const EnhancedAttendanceTable = ({
    ...
    handleEdit = () => {},
    handleDelete = () => {},
    generateRandomColor = (seed) => { // Ajustado para modo claro
        let hash = 0;
        if (!seed) return `hsl(0, 45%, 75%)`; // Color pastel por defecto
        for (let i = 0; i < seed.length; i++) {
            hash = seed.charCodeAt(i) + ((hash << 5) - hash);
            hash = hash & hash;
        }
        const H = hash % 360;
        return `hsl(${H}, 45%, 80%)`; // Tonos pastel (alta luminosidad, media saturación)
    },
    ...
    const handleSaveTarifa = async (profesorId) => {
        ...
        if (isNaN(nuevaTarifaNum) || nuevaTarifaNum < 0) {
            setEditingTarifas(prev => ({
                ...prev,
                [profesorId]: { ...tarifaEditState, error: "Valor inválido", guardando: false }
            }));
            return;
        }
        ...
        setEditingTarifas(prev => ({
            ...prev,
            [profesorId]: { ...tarifaEditState, guardando: true, error: null }
        }));
        ...
        try {
            ...
            setEditingTarifas(prev => ({
                ...prev,
                [profesorId]: { ...tarifaEditState, guardando: false, error: errorMessage }
            }));
        }
    };
    ...
    const renderTableHeader = () => {
        ...
        return (
            <TableHead>
                <TableRow sx={{ '& .MuiTableCell-root': { fontWeight: '600', borderBottom: `1px solid ${COLORS.divider}`, color: COLORS.textPrimary, bgcolor: alpha(COLORS.backgroundSubtle, 0.5) } }}> {/* Fondo cabecera ligeramente diferente */}
                    <TableCell sx={{ ...getSortableHeaderStyle('nombre'), minWidth: 180, position: 'sticky', left: 0, zIndex: 10, bgcolor: alpha(COLORS.backgroundSubtle, 0.5), borderRight: `1px solid ${COLORS.divider}` }} onClick={() => requestSort('nombre')}><Box sx={{ display: 'flex', alignItems: 'center' }}>Profesor {renderSortIcon('nombre')}</Box></TableCell>
                                        ...
                                        <React.Fragment key={profesor.id}>
                                            <motion.tr component={TableRow} custom={index} initial="hidden" animate="visible" exit="exit" variants={rowVariants} onMouseEnter={() => setHoveredRow(profesor.id)} onMouseLeave={() => setHoveredRow(null)} sx={{ '& > .MuiTableCell-root': { py: 0.4, px: 1.5, borderBottom: `1px solid ${COLORS.divider}`, fontSize:'0.8rem' }, '&:hover': { bgcolor: COLORS.backgroundSubtle }, transition: 'background-color 0.15s ease-in-out', bgcolor: COLORS.backgroundPaper }}>
                                                <TableCell component="th" scope="row" sx={{ cursor: 'pointer', position: 'sticky', left: 0, bgcolor: 'inherit', zIndex: 1, borderRight: `1px solid ${COLORS.divider}`, borderLeft: isExpanded ? `3px solid ${COLORS.accentGreen}` : 'none', color: COLORS.textPrimary }} onClick={() => toggleRowExpand(profesor.id)}>
                                                    <Box sx={{ display: 'flex', alignItems: 'center', py: 0.5 }}>
                                                        <Badge overlap="circular" anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} badgeContent={isExpanded ? <ExpandLessIcon sx={{ fontSize: '0.8rem', color: COLORS.accentGreen, bgcolor: alpha(COLORS.accentGreen, 0.15), borderRadius: '50%' }} /> : <ExpandMoreIcon sx={{ fontSize: '0.8rem', color: COLORS.textSecondary }} />}>
                                                            <Avatar sx={{ bgcolor: generateRandomColor(profesor.nombre), mr: 1.2, width: 32, height: 32, fontSize: '0.8rem', color: COLORS.textPrimary, /* Texto oscuro para avatar claro */ transition: 'transform 0.15s ease-in-out', transform: isExpanded ? 'scale(1.03)' : 'scale(1)' }}>{getInitials(profesor.nombre)}</Avatar>
                                                        </Badge>
                                                        <Box>
                                                            ...
                                                            {profesor.actividad && (<Tooltip title={`${profesor.actividad}`} placement="bottom-start" arrow><Typography variant="caption" noWrap sx={{fontSize: '0.65rem', display: 'flex', alignItems: 'center', mt: 0.2, fontStyle: 'italic', color: alpha(COLORS.accentGreen, 0.85) }}><ActivityIcon sx={{ fontSize: '0.8rem', mr: 0.3, opacity: 0.75 }} /> {profesor.actividad}</Typography></Tooltip>)}
                                                        </Box>
                                                    </Box>
                                                </TableCell>
                                                {diasLaborables.map(diaCompleto => (<TableCell key={`${profesor.id}_${diaCompleto}_horas`} align="center" sx={{color: COLORS.textPrimary}}>
                                                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0.2 }}>
                                                        <Tooltip title="Restar media hora" placement="top" arrow><IconButton onClick={() => handleQuickAddHours(profesor.id, diaCompleto, -0.5)} size="small" sx={{p:0.1, color: alpha(COLORS.accentGreen, 0.7), '&:hover': {color: COLORS.accentGreenHover, bgcolor: alpha(COLORS.accentGreen, 0.1)} }}><RemoveHourIcon sx={{fontSize:"1.1rem"}} /></IconButton></Tooltip>
                                                        <TextField type="number" size="small" variant="outlined" value={profesorAttendance[diaCompleto]?.horas ?? 0} onChange={(e) => handleHoursChange(profesor.id, diaCompleto, e.target.value)} inputProps={{ min: 0, step: 0.5, style: { textAlign: 'center', padding: '6px 2px', fontSize: '0.8rem' } }} sx={{ width: '55px', '& .MuiOutlinedInput-root': { height: '30px', borderRadius: '4px', bgcolor: alpha(COLORS.black, 0.03), '& input': { fontWeight: (profesorAttendance[diaCompleto]?.horas || 0) > 0 ? 'bold' : 'normal', color: (profesorAttendance[diaCompleto]?.horas || 0) > 0 ? COLORS.accentGreen : COLORS.textPrimary }, '& fieldset': { borderColor: alpha(COLORS.accentGreen, 0.35) }, '&:hover fieldset': { borderColor: alpha(COLORS.accentGreen, 0.6) }, '&.Mui-focused fieldset': { borderColor: COLORS.accentGreen }, } }} />
                                                        <Tooltip title="Sumar media hora" placement="top" arrow><IconButton onClick={() => handleQuickAddHours(profesor.id, diaCompleto, 0.5)} size="small" sx={{p:0.1, color: alpha(COLORS.accentGreen, 0.7), '&:hover': {color: COLORS.accentGreenHover, bgcolor: alpha(COLORS.accentGreen, 0.1)} }}><AddHourIcon sx={{fontSize:"1.1rem"}} /></IconButton></Tooltip>
                                                    </Box>
                                                </TableCell>))}
                                                ...
                                                <TableCell align="center"><Stack direction="row" spacing={0.2} justifyContent="center">
                                                    ...
                                                    <Tooltip title="Eliminar profesor" arrow><IconButton onClick={() => handleDelete(profesor.id, profesor.nombre)} sx={{p:0.5, color: COLORS.errorMain, '&:hover': { bgcolor: alpha(COLORS.errorMain, 0.08), color: COLORS.errorMain } }}><DeleteIcon sx={{fontSize:"1.1rem"}} /></IconButton></Tooltip>
                                                </Stack></TableCell>
                                            </motion.tr>
                                            
                                            {/* --- SECCIÓN COLAPSABLE CON EDICIÓN DE TARIFA --- */}
                                                ...
                                                <TableCell colSpan={3 + diasLaborables.length} sx={{ p: 0, borderBottom: isExpanded ? `1px solid ${COLORS.divider}` : 'none' }}>
                                                    <Collapse in={isExpanded} timeout={ {enter: 300, exit: 150} } unmountOnExit>
                                                        <Zoom in={isExpanded} style={{ transitionDelay: isExpanded ? '50ms' : '0ms' }}>
                                                            <Card elevation={0} sx={{ m: 0.5, p: 1.5, bgcolor: COLORS.backgroundSubtle, borderRadius: '4px', border: `1px solid ${alpha(COLORS.divider, 0.7)}` }}>
                                                                ...
                                                                {profesor.actividad && ( <Typography variant="body2" sx={{mb:1, display: 'flex', alignItems: 'center', color: COLORS.textSecondary, fontSize:'0.8rem'}}> <ActivityIcon sx={{mr:0.6, fontSize:'1rem', opacity:0.75, color: alpha(COLORS.accentGreen, 0.75) }}/><strong>Actividad:</strong>&nbsp;{profesor.actividad}</Typography>)}
                                                                ...
                                                                <Grid container spacing={1.5} alignItems="flex-start">
                                                                        ...
                                                                        {!tarifaState && onUpdateTarifaHora ? (
                                                                            ...
                                                                            <Box sx={{ mt: 0.1 }}>
                                                                                <TextField
                                                                                    ...
                                                                                    sx={{ width: '90px', mr: 0.5, '& .MuiOutlinedInput-root': { height: '30px', borderRadius: '4px', bgcolor: alpha(COLORS.black, 0.04), '& input': { color: COLORS.accentGreen, fontWeight: 'bold' }, '& fieldset': { borderColor: tarifaState.error ? COLORS.errorMain : alpha(COLORS.accentGreen, 0.5) }, '&:hover fieldset': { borderColor: tarifaState.error ? COLORS.errorMain :alpha(COLORS.accentGreen, 0.8) }, '&.Mui-focused fieldset': { borderColor: tarifaState.error ? COLORS.errorMain :COLORS.accentGreen }}}}
                                                                                />
                                                                                ...
                                                                                <Tooltip title="Cancelar"><span>
                                                                                    <IconButton size="small" onClick={() => handleCancelEditTarifa(profesor.id)} disabled={tarifaState.guardando} sx={{ p:0.3, color: COLORS.errorMain, '&:hover': { bgcolor: alpha(COLORS.errorMain, 0.1) } }}>
                                                                    ...
                                                                    {/* Fin Sección de Tarifa por Hora */}

                                                                    <Grid item xs={12} sm={6} md={3} sx={{display: 'flex', alignItems: 'flex-end', justifyContent: {xs: 'flex-start', sm: 'flex-end'}, mt: {xs: 1, md: 0}}}>
                                                                        <Button variant="outlined" size="small" onClick={() => onVerHistorialPagos(profesor)} startIcon={<HistoryIcon sx={{fontSize:'1rem'}}/>} sx={{ fontSize:'0.7rem', py:0.3, px:1, color: COLORS.accentGreen, borderColor: alpha(COLORS.accentGreen, 0.5), '&:hover': { borderColor: COLORS.accentGreen, bgcolor: alpha(COLORS.accentGreen, 0.08) } }}>Historial</Button>
                                                                    </Grid>
                                                                </Grid>
                                                                {totalHoras > 0 && (
                                                                    <Box sx={{mt: 1.5}}>
                                                                        <Typography variant="caption" sx={{color: COLORS.textSecondary, fontSize:'0.7rem'}}>Progreso semanal (ref. 40hs):</Typography>
                                                                        <LinearProgress variant="determinate" value={Math.min((totalHoras / 40) * 100, 100)} sx={{height: 6, borderRadius: 3, bgcolor: alpha(COLORS.accentGreen, 0.2), '& .MuiLinearProgress-bar': {bgcolor: COLORS.accentGreen}}} />
                                                                    </Box>
                                                                )}
                                                            </Card>
                                                        </Zoom>
