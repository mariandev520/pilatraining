...
// Componente SeccionHorarioFuncional mejorado
const SeccionHorarioFuncional = React.memo(({
    horario,
    diaSeleccionado,
    clientesAsignados = [],
    clienteSeleccionado,
        ...
        const cargarDatos = async () => {
            try {
                ...
                setState(prev => ({
                    ...
                    clientesFuncional: clientesFiltrados,
    ...
    if (state.loading) {
        return (
            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '80vh' }}>
                ...
                <Typography sx={{ ml: 2 }}>Cargando clases funcionales...</Typography>
            </Box>
        );
    }

    if (state.error && !Object.keys(state.asignaciones).length && state.clientesFuncional.length === 0) {
        return (
        ...
        <Container maxWidth="xl" sx={{ mt: 4, mb: 8 }}>
            ...
            <Box sx={{ mb: 4 }}>
                <Typography variant="h4" component="h1" gutterBottom align="center" sx={{ 
                    ...
                    Gestión de Clases Funcionales
                </Typography>
                <Typography variant="subtitle1" color="text.secondary" align="center">
                    Asigna múltiples clientes a cada horario de entrenamiento funcional
                </Typography>
            </Box>
                ...
                <Grid item xs={12} md={3}>
                    <Paper elevation={2} sx={{ 
                        ...
                        <Box sx={{ 
                            ...
                            {state.clientesFuncional.length > 0 ? (
                                <List dense sx={{ pt: 0 }}>
                                    {state.clientesFuncional.map(cliente => (
                                        <ClienteItem
                                            key={cliente.id}
                                            cliente={cliente}
